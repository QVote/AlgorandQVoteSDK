 ...
 */
async function buildDeployTxs(client: any, creatorAddress: string,
							 startTime: number, registrationSeconds: number,
							 assetId: number, assetCoefficient: number, 
							 decisionName: string, options: string[]) : Promise<any[]>{
	...
	const appCreateTx =  algosdk.makeApplicationCreateTxn(creatorAddress, params, onComplete, 
											...
											appArgs); 
	txs.push(appCreateTx)

	// Add option txs 
	groupedOptions.slice(1).map((opts) => )
	return txs;

}

// TODO compute all transactions needed to deploy a whole election. 
...
(async () => {
	try {
		...
		const txs = await buildDeployTxs(algodClient, creatorAccount.addr, 
