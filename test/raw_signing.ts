...
(async () => {
    ...
    const creatorMemo = 'strategy nurse quantum rookie depend away payment middle horse derive bleak fashion grid response wing garbage filter stairs proud timber devote cinnamon cover about marble'
    const creatorAccount = algosdk.mnemonicToSecretKey(creatorMemo) 

    const userMemo =  'leopard wrestle history fog scare twist churn bullet action poet enter ketchup gasp media bonus joke anger month defy degree grit witness strategy able income'
    ...
    async function deployNew() {
        try {
            ...
            const params = {
                ...
                options:  ["first", "second", "third", "hey", "anotherone", "six"],
            ...
            await Promise.all(signedAddOtionTxs.map(tx => qv.sendSignedTx(tx)))
            await Promise.all(addOptiontxIDs.map(txID => {qv.waitForConfirmation(txID)}))
    ...
    async function vote(appID: number){
        try {
            ...
            await Promise.all(voteTxIDs.map(txID => {qv.waitForConfirmation(txID)}))
